...
export class KeyManager {
  ...
  public async initialize(mnemonic: string): Promise<void> {
    ...
    try {
      ...
      throw new KeyManagerError(`Failed to initialize key manager: ${error}`, 'INITIALIZATION_FAILED');
    }
  }
  ...
  private derivePrivateKey(index: number): string {
    ...
    try {
      ...
      throw new KeyManagerError(`Failed to derive key at index ${index}: ${error}`, 'DERIVATION_FAILED');
    }
  }
  ...
  public async generateTradeKey(orderId: string): Promise<string> {
    ...
    try {
      ...
      throw new KeyManagerError(`Failed to generate trade key: ${error}`, 'TRADE_KEY_GENERATION_FAILED');
    }
  }
  ...
  public getPublicKeyFromPrivate(privateKey: string): string {
    try {
      ...
      throw new KeyManagerError(`Failed to get public key: ${error}`, 'PUBLIC_KEY_GENERATION_FAILED');
    }
  }
